...
// 262. Consuming Promises Async/await - 263. Error Handling
...
// Old Style
// whereAmI()
//   .then(info => console.log(`2: Output information ${info}`)) // If we don't throw (Line 560) an error, even if there is an error on the promise it will take it as good return
//   .catch(err => console.error(`2: There was an error ${err.message}`))
//   .finally(() => console.log('3: Finish getting the location'));

// Using Async and Await
(async function () {
  try {
    const countryInfo = await whereAmI();
    console.log(`2: Output information ${countryInfo}`);
  } catch (err) {
    console.error(`2: There was an error ${err.message}`);
  }
  console.log('3: Finish getting the location');
})();
...
const get3Countries = async function (c1, c2, c3) {
  try {
    ...
    // Work with multiple Promises, but if one fail, all fails
...
get3Countries('argentina', 'brazil', 'chile');

// 266. Other Promises Combinators
...
// Promise.any - Its like Promise.race but it will discard the error
Promise.any([
  Promise.resolve('Success'),
  Promise.reject('ERROR'),
